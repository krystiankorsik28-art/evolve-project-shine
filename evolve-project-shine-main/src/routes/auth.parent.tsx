...
function ParentAuth() {
  ...
  const submitRegister = async () => {
    if (!fname || !lname || !email || !pass || !phone) { toast.error("Fill all fields"); return; }
    setBusy(true);
    await new Promise((r) => setTimeout(r, 800));
    setBusy(false);
  ...
  return (
    <AuthLayout title="The AI Platform for Modern Education" subtitle="Monitor your child's progress with EduNex.">
      <Toaster theme="dark" />
      <h1 className="text-xl font-semibold text-white mb-1">Parent</h1>
      <p className="text-sm text-white/40 mb-6">Monitor your child&apos;s progress</p>
      ...
      {mode === "login" ? (
        ...
        <div className="space-y-3">
          ...
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email" className="w-full h-10 px-3 text-xs rounded-lg bg-white/[0.04] border border-white/[0.08] text-white placeholder:text-white/20 outline-none focus:border-cyan-500/40 transition-colors" />
          <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone" className="w-full h-10 px-3 text-xs rounded-lg bg-white/[0.04] border border-white/[0.08] text-white placeholder:text-white/20 outline-none focus:border-cyan-500/40 transition-colors" />
          <input type="password" value={pass} onChange={(e) => setPass(e.target.value)} placeholder="Password" className="w-full h-10 px-3 text-xs rounded-lg bg-white/[0.04] border border-white/[0.08] text-white placeholder:text-white/20 outline-none focus:border-cyan-500/40 transition-colors" />
