...
function TeacherPanel() {
      ...
      if (user) {
        ...
        setDisplayName(
          user.user_metadata?.display_name ??
            ...
            ((user.email ?? "").split("@")[0] || "nauczycielu"),
        );
      }
  ...
  const greet =
    now.getHours() < 12 ? "Dzień dobry" : now.getHours() < 18 ? "Dzień dobry" : "Dobry wieczór";

  const notifications = exams.slice(0, 5).map((e) => ({
    id: e.id,
    title: e.title,
    sub: e.status === "published" ? "Opublikowany" : "Wersja robocza",
    when: new Date(e.created_at).toLocaleString("pl-PL", {
      dateStyle: "short",
      timeStyle: "short",
    }),
  }));
  ...
  return (
    <div className="min-h-screen bg-[#0a0a12] text-slate-200 flex">
      ...
      <div className="flex flex-1 min-h-screen">
        {/* Sidebar */}
        <aside
          className={`${collapsed ? "w-[68px]" : "w-64"} transition-all duration-300 bg-[#0c0c16]/90 backdrop-blur-xl border-r border-white/[0.06] flex flex-col sticky top-0 h-screen`}
        >
          ...
          <nav className="flex-1 px-2 py-4 space-y-4 overflow-y-auto scrollbar-thin">
            {NAV_GROUPS.map((group) => (
              <div key={group.label}>
                {!collapsed && (
                  <div className="px-3 pb-1 text-[10px] tracking-[0.15em] text-white/25 font-mono uppercase">
                    {group.label}
                  </div>
                )}
                <div className="space-y-0.5">
                  {group.items.map((n) => {
                    ...
                    return (
                      ...
                      >
                        {active && !collapsed && (
                          <span className="absolute -left-2 top-1/2 -translate-y-1/2 w-0.5 h-5 rounded-r bg-cyan-400" />
                        )}
          ...
          <div className="p-3 border-t border-white/[0.06] space-y-2">
            {!collapsed ? (
              <div className="flex items-center gap-3 px-2 py-2 rounded-xl bg-white/[0.03] border border-white/[0.06]">
                <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-cyan-400 to-blue-500 grid place-items-center text-[#0a0a12] font-bold text-sm">
                  {userInitial}
                </div>
                <div className="min-w-0 flex-1">
                  <div className="text-[9px] tracking-[0.15em] text-cyan-400/60 font-mono">
                    TEACHER
                  </div>
                  <div className="text-xs text-white/70 truncate">
                    {email || "teacher@edunex.pl"}
                  </div>
                </div>
              </div>
            ) : (
              <div className="w-8 h-8 mx-auto rounded-lg bg-gradient-to-br from-cyan-400 to-blue-500 grid place-items-center text-[#0a0a12] font-bold text-sm">
                {userInitial}
              </div>
            )}
            <button
              onClick={logout}
              className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-white/30 hover:text-white/60 hover:bg-white/[0.04] transition ${collapsed ? "justify-center" : ""}`}
            >
        ...
        <main className="flex-1 min-w-0 flex flex-col">
          ...
          <div className="h-14 border-b border-white/[0.06] bg-[#0a0a12]/80 backdrop-blur-xl flex items-center justify-between px-4 lg:px-6 sticky top-0 z-40">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              >
                ...
                <span className="ml-3 inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-white/5 border border-white/10 text-[10px] font-mono text-white/40">
              ...
              <div className="relative" ref={notifRef}>
                ...
                >
                  ...
                  {notifications.length > 0 && (
                    <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-pink-500 ring-2 ring-[#0a0a12]" />
                  )}
                </button>
                {notifOpen && (
          ...
          {searchOpen && (
            <div
              className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm grid place-items-start pt-32 px-4"
              onClick={() => setSearchOpen(false)}
            >
              <div
                className="w-full max-w-xl rounded-2xl border border-white/10 bg-[#0b1224]/95 backdrop-blur-xl shadow-2xl overflow-hidden"
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="relative flex flex-col lg:flex-row lg:items-center gap-6 justify-between">
          <div className="space-y-3 max-w-xl">
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              ...
              <button
                onClick={() => go("live")}
                className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-sm transition"
              >
      ...
      <div className="flex flex-wrap gap-2 p-3 rounded-xl bg-gradient-to-r from-accent/5 via-accent/5 to-accent/5 border border-white/10">
        <span className="text-[10px] font-mono uppercase tracking-widest text-white/40 self-center mr-2">
          Szybkie tworzenie:
        </span>
        <button
          onClick={() => go("ai")}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-cyan-500/15 hover:bg-cyan-500/25 text-accent text-xs transition"
        >
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 rounded-2xl border border-white/[0.06] bg-white/[0.02] backdrop-blur p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="flex items-center gap-2 text-lg font-display font-bold text-white">
            ...
            <button
              onClick={() => go("egzaminy")}
              className="inline-flex items-center gap-1 text-xs text-accent hover:text-cyan-200 transition"
            >
        ...
        <div className="space-y-5">
          <div className="rounded-2xl border border-accent/20 bg-gradient-to-br from-accent/15 via-accent/5 to-accent/5 backdrop-blur p-6">
            <div className="flex items-center justify-between mb-2">
              <h3 className="flex items-center gap-2 text-base font-display font-bold text-accent">
                <Activity className="w-4 h-4" />
                Live monitoring
              </h3>
              <span className="text-[9px] font-mono tracking-wider px-1.5 py-0.5 rounded bg-accent/20 text-accent border border-accent/30">
                ACTIVE
              </span>
            </div>
            ...
            <button
              onClick={() => go("monitoring")}
              className="w-full inline-flex justify-center items-center gap-2 px-4 py-2.5 rounded-lg bg-accent hover:bg-accent/80 text-slate-900 font-semibold text-sm transition"
            >
          ...
          <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] backdrop-blur p-6">
            <h3 className="flex items-center gap-2 text-base font-display font-bold text-white mb-3">
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 rounded-2xl border border-white/[0.06] bg-white/[0.02] backdrop-blur p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="flex items-center gap-2 text-base font-display font-bold text-white">
        ...
        <div className="rounded-2xl border border-accent/20 bg-gradient-to-br from-accent/15 via-accent/10 to-accent/5 p-6">
          <h3 className="flex items-center gap-2 text-base font-display font-bold text-white mb-2">
          ...
          <div className="mt-4 text-[10px] font-mono text-white/30 tracking-wider">
            {email ? `ID · ${email}` : ""}
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] backdrop-blur p-6">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          <AIActionCard
            icon={Sparkles}
            title="AI Generator"
            desc="Generuj egzaminy w 60 sekund"
            action={() => go("ai")}
          ...
          <AIActionCard
            icon={Wand2}
            title="AI Ocenianie"
            desc="Automatyczna ocena esejów"
            action={() => go("aiocen")}
        ...
        <div className="mt-4 grid sm:grid-cols-3 gap-2">
          <div className="flex items-center gap-2 p-3 rounded-xl bg-white/[0.03] border border-white/[0.04]">
            ...
            <span className="text-xs text-white/50">AI Generator — gotowy</span>
          </div>
          <div className="flex items-center gap-2 p-3 rounded-xl bg-white/[0.03] border border-white/[0.04]">
            <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span className="text-xs text-white/50">AI Tutor — online</span>
          </div>
          <div className="flex items-center gap-2 p-3 rounded-xl bg-white/[0.03] border border-white/[0.04]">
            <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span className="text-xs text-white/50">AI Ocenianie — aktywne</span>
          </div>
        </div>
      </div>
    </div>
...
function AIActionCard({
  ...
  return (
    <button
      onClick={action}
      className="group flex items-start gap-3 p-4 rounded-xl bg-white/[0.03] border border-white/[0.06] hover:border-accent/20 hover:bg-white/[0.05] transition-all text-left"
    >
      <div
        className={`w-9 h-9 rounded-lg bg-gradient-to-br ${color} grid place-items-center shrink-0 group-hover:scale-110 transition`}
      >
        <Icon className="w-4 h-4 text-white" />
      </div>
      <div>
        <div className="text-sm font-semibold text-white group-hover:text-accent transition">
          {title}
        </div>
        ...
        return (
          <div key={i} className="flex-1 flex flex-col items-center gap-1.5 group">
            <div
              ...
              style={{ height: `${h}%` }}
            >
              <span className="absolute -top-5 left-1/2 -translate-x-1/2 text-[10px] font-mono text-white/60 opacity-0 group-hover:opacity-100 transition">
                {v}
              </span>
            </div>
...
function QuickAction({
  ...
  return (
    <button
      onClick={onClick}
      className="flex flex-col items-start gap-2 p-3 rounded-xl bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.06] hover:border-cyan-400/30 transition group"
    >
            ...
            <div className="flex items-center gap-3 mb-1">
              ...
              <div>
                ...
                <p className="text-xs text-white/50">
                  Gemini · pytania ze zdjęcia, z tematu, lub ilustracje do pytań — z zapisem do
                  banku lub bezpośrednio do egzaminu.
                </p>
              </div>
            </div>
          </div>
...
function TabBtn({
  ...
  return (
    <button
      onClick={onClick}
      className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-semibold transition ${active ? "bg-cyan-500 text-slate-900" : "bg-white/5 text-white/70 hover:bg-white/10 border border-white/10"}`}
    >
...
function ResultActions({ qs, single }: { qs: AiQ[]; single?: boolean }) {
  ...
  const bank = async () => {
    setBusy("bank");
    try {
      await saveQuestionsToBank(qs);
      toast.success(`Dodano do banku (${qs.length})`);
    } catch (e) {
  ...
  const exam = async () => {
    if (!examId) return toast.error("Wybierz egzamin");
    setBusy("exam");
    try {
      await saveQuestionsToExam(qs, examId);
      toast.success(`Dodano do egzaminu`);
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Błąd");
    } finally {
      setBusy(null);
  ...
  return (
    <div className="mt-3 flex flex-wrap items-center gap-2">
      <button
        disabled={busy !== null}
        onClick={bank}
        className="px-3 py-1.5 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-slate-900 text-xs font-semibold disabled:opacity-50"
      >
      ...
      <button
        disabled={busy !== null || !examId}
        onClick={exam}
        className="px-3 py-1.5 rounded-lg bg-cyan-500 hover:bg-cyan-400 text-slate-900 text-xs font-semibold disabled:opacity-50"
      >
    ...
    <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4 space-y-2">
      <div className="flex items-center justify-between">
        <span className="text-[10px] font-mono text-accent">
          PYT. {idx + 1} · {q.question_type.toUpperCase()} · {q.difficulty.toUpperCase()} ·{" "}
          {q.points}p
        </span>
      </div>
      <div className="text-sm text-white font-medium">{q.prompt}</div>
      {q.options && q.options.length > 0 && (
        <ul className="text-xs text-white/70 space-y-0.5 pl-4 list-disc">
          {q.options.map((o, i) => (
            <li key={i}>{o}</li>
          ))}
        </ul>
    ...
    <div className="grid lg:grid-cols-2 gap-5">
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 space-y-4">
        <label className="block">
          <span className="text-xs uppercase tracking-wider text-accent mb-2 block font-mono">
            Opis ilustracji
          </span>
          <textarea
            rows={5}
            ...
            placeholder="np. Schemat układu krwionośnego człowieka"
            className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg outline-none focus:border-cyan-400/50 text-white placeholder-white/30"
          />
        </label>
        <button
          disabled={loading}
          onClick={generate}
          className="w-full inline-flex justify-center items-center gap-2 bg-gradient-to-r from-accent to-blue-500 text-slate-900 font-bold px-5 py-3 rounded-xl disabled:opacity-50"
        >
          {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Wand2 className="w-4 h-4" />}{" "}
          {loading ? "Generuję..." : "Wygeneruj ilustrację"}
        </button>
      </div>
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
        ...
        {url ? (
          <div className="space-y-3">
            ...
            <a
              href={url}
              download="ilustracja.png"
              className="inline-block px-3 py-1.5 rounded-lg bg-emerald-500 text-slate-900 text-xs font-semibold"
            >
              Pobierz
            </a>
          </div>
...
function AIPhoto() {
  ...
  const generate = async () => {
    if (!file || !desc) {
      toast.error("Dodaj zdjęcie i opis");
      return;
    }
    setLoading(true);
    try {
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 space-y-4">
        <div>
          ...
          <label className="block border-2 border-dashed border-white/15 rounded-xl p-8 text-center cursor-pointer hover:border-cyan-400/40 hover:bg-white/[0.02] transition">
            ...
            />
            {preview ? (
              <img src={preview} alt="podgląd" className="max-h-64 mx-auto rounded-lg" />
            ) : (
              <>
                <ImageIcon className="w-10 h-10 mx-auto text-white/30 mb-2" />
                <p className="text-sm text-white/50">Kliknij lub przeciągnij zdjęcie zadania</p>
              </>
            )}
          </label>
        </div>
        <div>
          <div className="text-xs uppercase tracking-wider text-violet-300 mb-2 font-mono">
            Krok 2
          </div>
          <textarea
            rows={3}
            ...
            placeholder="np. Pytanie ABCD do klasy 2 LO o sile tarcia."
            className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg outline-none focus:border-cyan-400/50 text-white placeholder-white/30"
          />
        </div>
        <button
          disabled={loading}
          onClick={generate}
          className="w-full inline-flex justify-center items-center gap-2 bg-gradient-to-r from-accent to-blue-500 text-slate-900 font-bold px-5 py-3 rounded-xl disabled:opacity-50 transition hover:scale-[1.01]"
        >
          {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Wand2 className="w-4 h-4" />}{" "}
          {loading ? "AI pracuje..." : "Wygeneruj pytanie"}
        </button>
      </div>
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
        <h3 className="font-semibold text-white mb-3 flex items-center gap-2">
        ...
        {result ? (
          <>
            <QPreview q={result} idx={0} />
            <ResultActions qs={[result]} single />
          </>
        ) : (
...
function AIGenerate() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const fullTopic = `${topic}${subject ? ` (przedmiot: ${subject})` : ""}${gradeLevel ? ` (poziom: ${gradeLevel})` : ""}`;
      const out = await aiGenerateQuestions({ data: { topic: fullTopic, count, difficulty } });
      setResult(out as AiQ[]);
      setProgress(100);
      toast.success(`Wygenerowano ${(out as AiQ[]).length} pytań`);
      setStep(5);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Błąd AI");
    } finally {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-1">
        {steps.map((s, i) => (
          <div key={s.n} className="flex items-center gap-1 flex-1">
            <button
              onClick={() => s.n < step && !loading && setStep(s.n)}
              className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium transition-all w-full
                ${
                  step === s.n
                    ? "bg-accent/15 text-accent border border-accent/30 shadow-md shadow-accent/10"
                    : s.n < step
                      ? "bg-emerald-500/10 text-emerald-300 border border-emerald-400/20 cursor-pointer hover:bg-emerald-500/15"
            ...
            >
              <span
                ...
                ${step === s.n ? "bg-accent text-slate-900" : s.n < step ? "bg-emerald-500/30 text-emerald-200" : "bg-white/10 text-white/30"}`}
              >
                {s.n < step ? "✓" : s.n}
              </span>
      ...
      {step === 1 && (
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 space-y-4">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-violet-500 to-purple-600 grid place-items-center">
              <BookOpen className="w-4 h-4 text-white" />
            </div>
            <div>
              <h3 className="text-base font-bold text-white">Wybierz przedmiot</h3>
              <p className="text-xs text-white/40">
                Do jakiego przedmiotu chcesz wygenerować pytania?
              </p>
            </div>
          </div>
            ...
            {SUBJECTS.map((s) => (
              <button
                ...
                className={`px-4 py-3 rounded-xl text-sm font-medium transition-all border
                  ${subject === s ? "bg-accent/15 text-accent border-accent/30 shadow-md shadow-accent/10" : "bg-white/[0.03] text-white/60 border-white/[0.06] hover:bg-white/[0.06] hover:text-white"}`}
              >
                {s}
              </button>
            ))}
      ...
      {step === 2 && (
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 space-y-4">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-emerald-500 to-teal-600 grid place-items-center">
              <Users className="w-4 h-4 text-white" />
            </div>
            <div>
              <h3 className="text-base font-bold text-white">Wybierz poziom klasy</h3>
              <p className="text-xs text-white/40">Dla jakiego poziomu edukacyjnego?</p>
            </div>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {GRADES.map((g) => (
              <button
                ...
                className={`px-4 py-3 rounded-xl text-sm font-medium transition-all border
                  ${gradeLevel === g ? "bg-accent/15 text-accent border-accent/30 shadow-md shadow-accent/10" : "bg-white/[0.03] text-white/60 border-white/[0.06] hover:bg-white/[0.06] hover:text-white"}`}
              >
                {g}
              </button>
            ))}
      ...
      {step === 3 && (
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 space-y-4">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-amber-500 to-orange-600 grid place-items-center">
              <FileText className="w-4 h-4 text-white" />
            </div>
            <div>
              <h3 className="text-base font-bold text-white">Typ pytań</h3>
              <p className="text-xs text-white/40">Jaki typ pytań chcesz wygenerować?</p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            {(["mixed", "multiple_choice", "true_false", "short_answer"] as const).map((t) => {
              const labels: Record<string, { name: string; desc: string }> = {
                mixed: { name: "Mieszane", desc: "Różne typy pytań" },
                multiple_choice: { name: "Wielokrotnego wyboru", desc: "ABCD — jeden poprawny" },
                true_false: { name: "Prawda/Fałsz", desc: "Tak lub Nie" },
                short_answer: { name: "Krótka odpowiedź", desc: "Otwarte pytania" },
              };
              return (
                <button
                  key={t}
                  ...
                  className={`p-4 rounded-xl text-left transition-all border
                    ${examType === t ? "bg-accent/15 border-accent/30 shadow-md shadow-accent/10" : "bg-white/[0.03] border-white/[0.06] hover:bg-white/[0.06]"}`}
                >
                  <div
                    className={`text-sm font-semibold ${examType === t ? "text-accent" : "text-white/70"}`}
                  >
                    {labels[t].name}
                  </div>
                  <div className="text-xs text-white/40 mt-0.5">{labels[t].desc}</div>
                </button>
              );
            })}
          </div>
      ...
      {step === 4 && (
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 space-y-4">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-accent to-blue-600 grid place-items-center">
              <Brain className="w-4 h-4 text-white" />
            </div>
            <div>
              <h3 className="text-base font-bold text-white">Szczegóły generowania</h3>
              <p className="text-xs text-white/40">Podaj temat i preferencje dotyczące pytań.</p>
            </div>
          </div>
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-4 mb-2">
            <div className="flex flex-wrap gap-2 text-xs">
              <span className="px-2 py-1 rounded-md bg-violet-500/10 text-violet-300 border border-violet-400/20">
                {subject}
              </span>
              <span className="px-2 py-1 rounded-md bg-emerald-500/10 text-emerald-300 border border-emerald-400/20">
                {gradeLevel}
              </span>
              <span className="px-2 py-1 rounded-md bg-amber-500/10 text-amber-300 border border-amber-400/20">
                {examType === "mixed"
                  ? "Mieszane"
          ...
          <label className="block">
            <span className="text-xs uppercase tracking-wider text-accent mb-2 block font-mono">
              Temat / zakres materiału
            </span>
            <textarea
              value={topic}
              onChange={(e) => setTopic(e.target.value)}
              rows={3}
              placeholder="np. II wojna światowa — przyczyny i przebieg, układ krwionośny człowieka, równania kwadratowe..."
              className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg outline-none focus:border-cyan-400/50 text-white placeholder-white/30 resize-none"
            />
          </label>
          <div className="grid grid-cols-2 gap-3">
            <label className="block">
              <span className="text-xs uppercase tracking-wider text-violet-300 mb-2 block font-mono">
                Liczba pytań
              </span>
            ...
            <label className="block">
              <span className="text-xs uppercase tracking-wider text-emerald-300 mb-2 block font-mono">
                Trudność
              </span>
                ...
                {(["easy", "medium", "hard"] as const).map((d) => (
                  <button
                    ...
                    className={`flex-1 py-2.5 rounded-lg text-xs font-semibold transition border
                      ${difficulty === d ? "bg-accent/15 text-accent border-accent/30" : "bg-white/[0.03] text-white/50 border-white/[0.06] hover:bg-white/[0.06]"}`}
                  >
                    {d === "easy" ? "Łatwa" : d === "medium" ? "Średnia" : "Trudna"}
                  </button>
                ))}
              </div>
            </label>
          ...
          {loading && (
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span className="text-accent font-mono">AI generuje pytania...</span>
                <span className="text-white/40 font-mono">{Math.round(progress)}%</span>
              </div>
              <div className="h-2 rounded-full bg-white/[0.06] overflow-hidden">
                <div
                  className="h-full bg-gradient-to-r from-accent to-blue-500 rounded-full transition-all duration-300"
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold text-white flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-accent" />
              Wygenerowane pytania ({result.length})
            </h3>
            <button
              onClick={() => {
                setStep(4);
      ...
      {step < 5 && (
        <div className="flex items-center justify-between">
          <button
            ...
            disabled={step === 1}
            className="px-4 py-2.5 rounded-lg text-sm font-medium text-white/50 bg-white/[0.03] border border-white/[0.06] hover:bg-white/[0.06] disabled:opacity-30 transition"
          >
            Wstecz
          </button>
          {step < 4 ? (
            <button
              ...
              disabled={!canGoNext()}
              className="px-5 py-2.5 rounded-lg text-sm font-semibold bg-gradient-to-r from-accent to-blue-500 text-slate-900 disabled:opacity-40 hover:brightness-110 transition inline-flex items-center gap-2"
            >
            ...
            >
              ...
              {loading ? "Generuję..." : "Wygeneruj pytania"}
            </button>
          )}
        </div>
      )}
...
function Certyfikaty() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (data)
        setAttempts(data.map((a) => ({ ...a, exam_title: titleMap[a.exam_id] ?? "Nieznany" })));
      setLoading(false);
    })();
  }, []);
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-8 text-center">
          ...
          <p className="text-xs text-white/40 mt-1">
            Certyfikaty pojawią się, gdy uczniowie zaliczą egzaminy.
          </p>
        </div>
      ) : (
                    ...
                    <tr key={a.id} className="hover:bg-white/[0.01] transition">
                      ...
                      <td className="p-4">
                        <span className="text-emerald-300 font-mono">
                          {a.score}/{a.max_score} ({Math.round(a.percent)}%)
                        </span>
                      </td>
                      <td className="p-4 text-white/50 text-[11px]">
                        {a.submitted_at
                          ? new Date(a.submitted_at).toLocaleDateString("pl-PL")
                          : "-"}
                      </td>
                      <td className="p-4">
