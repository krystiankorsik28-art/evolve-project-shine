...
const examResults: ExamResult[] = [
  ...
  {
    id: "e2",
    examTitle: "Język polski - lektury",
  ...
  {
    id: "e4",
    examTitle: "Język angielski - grammar",
...
const notices: Notice[] = [
  {
    id: "a1",
    type: "warning",
    message: "Kacper ma do powtórzenia materiał z fizyki przed kolejnym sprawdzianem.",
    date: "2026-06-28",
    child: "Kacper",
  },
  {
    id: "a2",
    type: "success",
    message: "Zuzanna poprawiła wynik z języka angielskiego o 15 punktów procentowych.",
    date: "2026-06-26",
    child: "Zuzanna",
  },
  {
    id: "a3",
    type: "info",
    message: "Nowy sprawdzian z chemii zaplanowano na przyszły tydzień.",
    date: "2026-06-24",
    child: "Kacper",
  },
];
...
function ParentPanel() {
  ...
  const filteredChildren = useMemo(() => {
    return selectedChild === "all"
      ? childrenData
      : childrenData.filter((child) => child.id === selectedChild);
  }, [selectedChild]);
  ...
  const averageScore = Math.round(
    examResults.reduce((sum, result) => sum + result.percent, 0) / examResults.length,
  );
  const passedCount = examResults.filter((result) => result.passed).length;
  const averageAttendance = Math.round(
    childrenData.reduce((sum, child) => sum + child.attendance, 0) / childrenData.length,
  );
    ...
    <div className="edunex-next-gen-panel edunex-parent-workspace min-h-screen bg-[#f6f8fb] text-slate-950">
      ...
      <main className="workspace-main mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <motion.section
          ...
          className="workspace-hero parent-hero overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm"
        >
          <div className="grid gap-px bg-slate-200 lg:grid-cols-[1fr_420px]">
            <div className="bg-slate-950 px-6 py-8 text-white">
              ...
              <h1 className="max-w-3xl text-3xl font-semibold sm:text-4xl">
                Spokojny przegląd wyników, frekwencji i komunikatów.
              </h1>
              <p className="mt-4 max-w-2xl text-sm leading-6 text-slate-300">
                Rodzic widzi najważniejsze informacje bez rozbudowanej administracji i bez dostępu
                do danych innych uczniów.
              </p>
            </div>
          ...
          <aside className="space-y-6">
            <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <div className="space-y-2">
                <ChildButton
                  active={selectedChild === "all"}
                  onClick={() => setSelectedChild("all")}
                  label="Wszystkie dzieci"
                  icon={Users}
                />
                {childrenData.map((child) => (
                  ...
                  >
                    <div className="grid h-9 w-9 place-items-center rounded-full bg-slate-950 text-xs font-semibold text-white">
                      {child.initials}
                    </div>
                    <div>
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="grid h-11 w-11 place-items-center rounded-full bg-slate-950 text-sm font-semibold text-white">
            {child.initials}
          </div>
          <div>
      ...
      <div className="mt-5 grid grid-cols-3 gap-3">
        ...
        <MiniStat
          label="Zaliczone"
          value={`${child.examsPassed}/${child.examsTotal}`}
          icon={Award}
        />
      ...
      {visible.map((item) => (
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 text-xs text-slate-500">
              {item.child} · {new Date(item.date).toLocaleDateString("pl-PL")}
            </div>
          </div>
