...
import { ArrowLeft, ArrowRight, BookOpenCheck, Brain, FileText, GraduationCap, LayoutDashboard, LockKeyhole, ShieldCheck } from "lucide-react";
...
export const Route = createFileRoute("/moduly")({
  ...
  head: () => ({
    meta: [
      { title: "Moduły — EduNex" },
      { name: "description", content: "Centrum modułów EduNex: logowanie, E-dziennik, dokumenty, AI Tutor i panele systemowe." },
    ],
  }),
});
      ...
      <main className="mx-auto max-w-7xl px-5 py-8 sm:px-8 lg:px-10">
        ...
        <section className="rounded-[32px] border border-white/10 bg-white/[.055] p-6 shadow-[0_32px_120px_rgba(0,0,0,.38)] backdrop-blur-xl lg:p-10">
          <div className="max-w-3xl">
            ...
            <h1 className="text-4xl font-semibold tracking-[-.055em] sm:text-5xl lg:text-6xl">Centrum modułów EduNex.</h1>
            <p className="mt-5 text-base leading-7 text-white/68 sm:text-lg">
              Szybkie wejście do najważniejszych części systemu: logowania, E-dziennika, dokumentów szkoły/IOD oraz diagnostyki AI Tutor.
            </p>
          </div>
        </section>
        ...
        <section className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {modules.map((item) => {
            ...
            const card = (
              <article className="h-full rounded-[28px] border border-white/10 bg-white/[.048] p-5 shadow-[0_24px_80px_rgba(0,0,0,.22)] backdrop-blur-xl transition hover:-translate-y-1 hover:border-white/18">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className="rounded-full border border-white/10 bg-white/[.04] px-3 py-1 text-[11px] font-semibold uppercase tracking-[.12em] text-white/55">{item.label}</span>
                </div>
            ...
            return isApi ? (
              <a key={item.href} href={item.href}>{card}</a>
            ) : (
              <Link key={item.href} to={item.href}>{card}</Link>
            );
          })}
        </section>
        ...
        <section className="mt-6 rounded-[28px] border border-white/10 bg-white/[.045] p-5 backdrop-blur-xl">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-white/58">Landing pokazuje wartość, logowanie daje dostęp, a moduły prowadzą do realnych funkcji szkoły.</p>
            </div>
            <Link to="/auth/teacher" className="inline-flex items-center justify-center gap-2 rounded-xl bg-white px-5 py-3 font-semibold text-[#07111f] hover:bg-white/90">
