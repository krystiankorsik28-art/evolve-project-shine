...
function RegisterPage() {
  return (
    <div className="min-h-screen bg-[#f7f8fb] px-4 py-10 text-[#0f172a]">
      <div className="mx-auto max-w-3xl rounded-[34px] border border-slate-200 bg-white p-8 shadow-[0_30px_100px_rgba(15,23,42,.10)]">
        ...
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="mt-4 text-base leading-7 text-slate-600">Wypełnij pełny formularz rejestracyjny. Bez przełączania dodatkowych zakładek.</p>
        </motion.div>
        <form className="mt-8 grid gap-4">
          <div className="grid gap-4 sm:grid-cols-2"><Input label="Imię" /><Input label="Nazwisko" /></div>
          <Input label="Adres e-mail" type="email" />
          <div className="grid gap-4 sm:grid-cols-2"><Input label="Telefon" /><Input label="Rola: uczeń / rodzic / nauczyciel / admin" /></div>
          <div className="grid gap-4 sm:grid-cols-2"><Input label="Nazwa szkoły" /><Input label="Klasa / stanowisko" /></div>
