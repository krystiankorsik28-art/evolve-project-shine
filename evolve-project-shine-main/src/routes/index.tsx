...
export const Route = createFileRoute("/")({
  ...
  head: () => ({
    meta: [
      { title: "EduNex â€” GOD MODE platforma egzaminacyjna" },
      {
        name: "description",
        content:
          "EduNex to nowoczesna platforma egzaminacyjna dla szkĂłĹ‚: sprawdziany, egzaminy, PIN ucznia, panel nauczyciela, AI, animacje, raporty, cennik i bezpieczeĹ„stwo.",
      },
    ],
  }),
});
...
const navLinks = [
  ...
  ["Panele", "#panele"],
  ["AI", "#ai"],
  ["BezpieczeĹ„stwo", "#bezpieczenstwo"],
  ["Cennik", "#cennik"],
  ["FAQ", "#faq"],
];
...
const platformBlocks: { icon: IconType; title: string; text: string }[] = [
  { icon: ClipboardList, title: "Egzaminy online", text: "TwĂłrz testy, sprawdziany, kartkĂłwki i egzaminy koĹ„cowe z punktacjÄ… oraz limitem czasu." },
  { icon: Hash, title: "Sesje PIN", text: "UczeĹ„ wpisuje imiÄ™, nazwisko i 6-cyfrowy kod. Bez zbÄ™dnych kont przy szybkim egzaminie." },
  { icon: Library, title: "Biblioteka pytaĹ„", text: "Gotowe zestawy, wĹ‚asne pytania, kategorie, poziomy trudnoĹ›ci i ponowne uĹĽywanie materiaĹ‚Ăłw." },
  { icon: BarChart3, title: "Raporty live", text: "Wyniki, Ĺ›rednia, czas pracy, postÄ™p klasy i szybka analiza problematycznych zadaĹ„." },
  { icon: Cloud, title: "Multimedia", text: "Pytania z obrazem, plikiem, audio, wideo i materiaĹ‚ami pomocniczymi w storage." },
  { icon: BadgeCheck, title: "Certyfikaty", text: "Po zakoĹ„czeniu testu system moĹĽe wygenerowaÄ‡ potwierdzenie, PDF albo numer weryfikacyjny." },
];
...
const panels = [
  {
    ...
    features: ["WejĹ›cie kodem PIN", "Timer i pasek postÄ™pu", "Pytania z multimediami", "Ekran wyniku"],
  },
  {
    icon: Users,
    title: "Panel nauczyciela",
    subtitle: "Centrum dowodzenia klasÄ…",
    features: ["Generator sesji", "PodglÄ…d odpowiedzi", "Biblioteka egzaminĂłw", "Eksport wynikĂłw"],
  },
  {
    icon: Building2,
    title: "Panel administratora",
    subtitle: "Kontrola szkoĹ‚y i bezpieczeĹ„stwa",
    features: ["Role i uprawnienia", "2FA i logi", "ZarzÄ…dzanie klasami", "Audyt aktywnoĹ›ci"],
  },
];

const workflow = [
  { nr: "01", title: "Projektujesz", text: "Dodajesz pytania, multimedia, punktacjÄ™, limit czasu i ustawienia dostÄ™pnoĹ›ci." },
  { nr: "02", title: "Uruchamiasz", text: "System tworzy sesjÄ™ i kod PIN dla uczniĂłw lub grupy klasowej." },
  { nr: "03", title: "Monitorujesz", text: "Nauczyciel widzi postÄ™p, aktywnoĹ›Ä‡, czas i oddane prace w czasie rzeczywistym." },
  { nr: "04", title: "Analizujesz", text: "Raport pokazuje wyniki, Ĺ›rednie, trudne pytania i rekomendacje do powtĂłrki." },
];

const aiCards: { icon: IconType; title: string; text: string }[] = [
  { icon: Bot, title: "Generator pytaĹ„", text: "AI tworzy pytania do tematu, klasy, poziomu i wymaganego typu odpowiedzi." },
  { icon: BrainCircuit, title: "Ocena opisowa", text: "Pomoc przy esejach, kryteriach, punktacji i komentarzach dla ucznia." },
  { icon: Lightbulb, title: "Rekomendacje", text: "System wskazuje dziaĹ‚y, ktĂłre wymagajÄ… powtĂłrzenia po sprawdzianie." },
];

const securityItems = [
  "2FA dla administratora i nauczyciela",
  "OTP resetu hasĹ‚a przez e-mail",
  "Powiadomienia o nowym logowaniu",
  "Role: uczeĹ„, nauczyciel, rodzic, admin",
  "Audyt zdarzeĹ„ i aktywnoĹ›ci",
  "RODO-ready architektura danych",
  "Storage dla plikĂłw egzaminacyjnych",
  "Ograniczenia sesji i kodĂłw PIN",
];

const modules: { icon: IconType; label: string }[] = [
  { icon: FileText, label: "Pytania wyboru" },
  { icon: CheckCircle2, label: "Prawda / faĹ‚sz" },
  { icon: BookOpen, label: "Eseje" },
  { icon: Timer, label: "Limity czasu" },
  { icon: Eye, label: "PodglÄ…d live" },
...
const plans: Plan[] = [
  { name: "Klasa", monthly: 0, tag: "Darmowy start", desc: "Dla jednej klasy lub szybkich testĂłw prĂłbnych.", limits: "1 nauczyciel Â· podstawowe limity", cta: "Zacznij za darmo", features: ["Sesje PIN", "Podstawowe wyniki", "Proste sprawdziany", "Panel ucznia"] },
  { name: "Korepetytor", monthly: 49, tag: "Indywidualnie", desc: "Dla korepetytorĂłw i maĹ‚ych grup.", limits: "do 60 uczniĂłw miesiÄ™cznie", cta: "Wybierz Korepetytor", features: ["Biblioteka testĂłw", "Raport ucznia", "Eksport wynikĂłw", "Branding podstawowy"] },
  { name: "Nauczyciel", monthly: 99, tag: "Najpopularniejszy", desc: "Najlepszy pakiet dla aktywnego nauczyciela.", limits: "do 250 uczniĂłw miesiÄ™cznie", cta: "Wybierz Nauczyciel", featured: true, features: ["AI generator pytaĹ„", "Wyniki live", "Sprawdziany i egzaminy", "Eksport PDF/CSV", "Sesje klasowe"] },
  { name: "SzkoĹ‚a", monthly: 490, tag: "PlacĂłwka", desc: "Dla szkoĹ‚y z wieloma klasami i nauczycielami.", limits: "do 25 nauczycieli", cta: "Wybierz SzkoĹ‚a", features: ["Panel admina", "Role i klasy", "2FA", "Raporty zbiorcze", "Wiele bibliotek"] },
  { name: "SzkoĹ‚a Plus", monthly: 890, tag: "Premium", desc: "Dla placĂłwek, ktĂłre chcÄ… peĹ‚ny system i AI.", limits: "do 80 nauczycieli", cta: "Wybierz Plus", features: ["AI rozszerzone", "Audyt zdarzeĹ„", "Certyfikaty", "Priorytet wsparcia", "WiÄ™kszy storage"] },
  { name: "Dzielnica", monthly: 2990, tag: "SieÄ‡ szkĂłĹ‚", desc: "Dla wielu placĂłwek zarzÄ…dzanych centralnie.", limits: "wiele szkĂłĹ‚ i adminĂłw", cta: "Kontakt wdroĹĽeniowy", features: ["Multi-school", "Panel centralny", "Raporty porĂłwnawcze", "SLA", "Onboarding"] },
  { name: "Kuratorium", monthly: null, tag: "Enterprise", desc: "NajwiÄ™kszy wariant dla instytucji i wdroĹĽeĹ„ specjalnych.", limits: "indywidualne limity i umowa", cta: "UmĂłw rozmowÄ™", features: ["Dedykowana konfiguracja", "Integracje", "ZgodnoĹ›Ä‡ formalna", "Raporty strategiczne", "Wsparcie premium"] },
];

const faqs = [
  ["Czy cennik jest juĹĽ normalny?", "Tak. Strona ma teraz 7 pakietĂłw: Klasa, Korepetytor, Nauczyciel, SzkoĹ‚a, SzkoĹ‚a Plus, Dzielnica i Kuratorium."],
  ["Czy jest przeĹ‚Ä…cznik miesiÄ™cznie / rocznie?", "Tak. Roczny wariant pokazuje rabat i obniĹĽonÄ… cenÄ™ w pakietach pĹ‚atnych."],
  ["Czy uczeĹ„ musi mieÄ‡ konto?", "Nie. MoĹĽe wejĹ›Ä‡ szybkim kodem PIN, jeĹ›li nauczyciel uruchomi sesjÄ™ egzaminacyjnÄ…."],
  ["Czy moĹĽna pĂłĹşniej podpiÄ…Ä‡ pĹ‚atnoĹ›ci?", "Tak. Sekcja cennika jest przygotowana pod przyciski zakupu lub kontakt z wdroĹĽeniem."],
];
  ...
  return (
    <motion.div className="fixed inset-0 z-[120] grid place-items-center overflow-hidden bg-[#020617] text-white" initial={{ opacity: 1 }} animate={{ opacity: 1 }}>
      ...
      <motion.div className="relative z-10 mx-5 max-w-5xl rounded-[36px] border border-white/10 bg-white/[0.055] p-8 text-center shadow-[0_40px_140px_rgba(0,0,0,0.55)] backdrop-blur-2xl sm:p-12" initial={{ scale: 0.88, y: 30, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }}>
        ...
        <h2 className="mt-5 text-4xl font-semibold tracking-[-0.07em] sm:text-7xl">EDUNEX PREMIUM START</h2>
        ...
        <button onClick={onSkip} className="mt-6 rounded-full border border-white/10 px-5 py-2 text-xs font-semibold text-white/55 transition hover:bg-white/10 hover:text-white">PomiĹ„ animacjÄ™</button>
      </motion.div>
    </motion.div>
  );
}
  ...
  return (
    <motion.div className="fixed inset-0 z-[115] grid place-items-center overflow-hidden bg-[#020617] text-white" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.18 }}>
      ...
      <div className="relative z-10 text-center">
        ...
        <div className="mt-3 text-3xl font-semibold tracking-[-0.04em]">Ĺadowanie panelu logowania</div>
        <p className="mt-2 text-sm text-white/55">PrzenoszÄ™ do bezpiecznej bramy EduNex...</p>
      </div>
    </motion.div>
  );
}
    ...
    <section className="mx-auto flex min-h-[94vh] max-w-7xl flex-col justify-center px-5 pb-20 pt-32 sm:px-8 lg:px-10">
      <div className="grid items-center gap-14 lg:grid-cols-[1.02fr_0.98fr]">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
          <Badge isLight={isLight} icon={ShieldCheck}>PaĹ„stwowa jakoĹ›Ä‡ dla szkoĹ‚y</Badge>
          <motion.h1 className="mt-7 max-w-5xl text-5xl font-semibold leading-[0.94] tracking-[-0.075em] sm:text-7xl lg:text-[88px]" initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18, duration: 0.7 }}>EduNex wchodzi w tryb premium.</motion.h1>
          <p className={`mt-7 max-w-2xl text-lg leading-8 sm:text-xl ${palette.muted}`}>Animowany landing premium, jasny motyw zgodny z panelem logowania, systemowe tĹ‚o, wejĹ›ciĂłwka 8K UHD i pĹ‚ynne przejĹ›cie do panelu logowania.</p>
          <div className="mt-9 flex flex-col gap-3 sm:flex-row"><button onClick={goAuth} className="group inline-flex items-center justify-center gap-2 rounded-full bg-[#0078d4] px-6 py-3.5 text-sm font-semibold text-white shadow-[0_18px_42px_rgba(0,120,212,0.30)] transition hover:bg-[#106ebe]">OtwĂłrz panel <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" /></button><a href="#cennik" className={`inline-flex items-center justify-center gap-2 rounded-full border px-6 py-3.5 text-sm font-semibold backdrop-blur transition ${palette.chip}`}>Zobacz cennik <BadgeCheck className="h-4 w-4" /></a></div>
  ...
  return (
    <div className={`relative overflow-hidden rounded-[34px] border p-4 backdrop-blur-2xl ${palette.card}`}>
      ...
      <div className="relative rounded-[28px] border border-white/10 bg-[#020817] p-5 text-white shadow-2xl">
        ...
        <div className="mt-6 rounded-3xl border border-white/10 bg-white/[0.04] p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs uppercase tracking-[0.22em] text-white/38">Aktywna sesja</p><h3 className="mt-2 text-2xl font-semibold">Sprawdzian â€” Matematyka</h3></div><motion.div className="rounded-2xl bg-[#0078d4] px-5 py-3 text-2xl font-bold shadow-[0_0_40px_rgba(0,120,212,0.45)]" animate={{ scale: [1, 1.04, 1] }} transition={{ duration: 1.8, repeat: Infinity }}>482 913</motion.div></div><div className="mt-5 h-2 rounded-full bg-white/10"><motion.div className="h-full rounded-full bg-[#50e6ff]" initial={{ width: "26%" }} animate={{ width: "82%" }} transition={{ duration: 2.8, repeat: Infinity, repeatType: "reverse" }} /></div></div>
        <div className="mt-4 grid grid-cols-3 gap-3">{[["28", "uczniĂłw"], ["87%", "Ĺ›rednia"], ["11m", "czas"]].map(([a, b], i) => <motion.div key={b} className="rounded-2xl border border-white/10 bg-white/[0.04] p-4" animate={{ y: [0, -4, 0] }} transition={{ duration: 3, repeat: Infinity, delay: i * 0.25 }}><div className="text-2xl font-semibold">{a}</div><div className="text-xs text-white/45">{b}</div></motion.div>)}</div>
        <div className="mt-4 grid gap-3">{["AI wykryĹ‚o trudne zadanie nr 7", "4 uczniĂłw wymaga dogrywki", "Raport PDF gotowy do eksportu"].map((x, i) => <motion.div key={x} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.04] p-3 text-sm text-white/72" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.7 + i * 0.12 }}><CheckCircle2 className="h-4 w-4 text-emerald-400" /> {x}</motion.div>)}</div>
      </div>
    </div>
  );
}
...
function PlatformSection({ palette, isLight }: { palette: Palette; isLight: boolean }) {
  return <section id="platforma" className="relative px-5 py-24 sm:px-8 lg:px-10"><SectionHeader isLight={isLight} icon={Layers} eyebrow="Platforma" title="CaĹ‚a strona pokazuje teraz prawdziwy system" text="Nie tylko Ĺ‚adny hero. Landing ma opisy moduĹ‚Ăłw, proces, role, AI, bezpieczeĹ„stwo i sensowny cennik." /><div className="mx-auto grid max-w-7xl gap-4 md:grid-cols-2 lg:grid-cols-3">{platformBlocks.map(({ icon: Icon, title, text }, i) => <motion.div whileHover={{ y: -8, scale: 1.015 }} key={title} className={`rounded-[28px] border p-6 backdrop-blur-2xl ${palette.card}`} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ delay: i * 0.05 }}><div className="grid h-12 w-12 place-items-center rounded-2xl bg-[#0078d4] text-white"><Icon className="h-5 w-5" /></div><h3 className="mt-6 text-xl font-semibold tracking-tight">{title}</h3><p className={`mt-3 text-sm leading-6 ${palette.muted}`}>{text}</p></motion.div>)}</div></section>;
}
...
function PricingSection({ palette, isLight, annual, setAnnual, goAuth }: { palette: Palette; isLight: boolean; annual: boolean; setAnnual: (v: boolean) => void; goAuth: () => void }) {
  const price = (monthly: number | null) => {
    ...
    if (monthly === 0) return "0 zĹ‚";
    return `${annual ? Math.round(monthly * 0.8) : monthly} zĹ‚`;
  };
  return <section id="cennik" className="px-5 py-24 sm:px-8 lg:px-10"><SectionHeader isLight={isLight} icon={BadgeCheck} eyebrow="Cennik" title="Cennik naprawiony â€” teraz wyglÄ…da jak SaaS premium" text="Siedem pakietĂłw, roczny rabat, wyrĂłĹĽniony plan i jasny opis, dla kogo jest kaĹĽdy wariant." /><div className={`mx-auto mb-8 flex max-w-7xl flex-col items-center justify-between gap-4 rounded-[28px] border p-4 sm:flex-row ${palette.card2}`}><div><div className="font-semibold">Rozliczenie</div><div className={`text-sm ${palette.muted}`}>Rocznie pokazuje okoĹ‚o 20% taniej dla pakietĂłw pĹ‚atnych.</div></div><div className={`grid grid-cols-2 rounded-full border p-1 ${palette.chip}`}><button onClick={() => setAnnual(false)} className={`rounded-full px-5 py-2 text-sm font-semibold ${!annual ? "bg-[#0078d4] text-white" : ""}`}>MiesiÄ™cznie</button><button onClick={() => setAnnual(true)} className={`rounded-full px-5 py-2 text-sm font-semibold ${annual ? "bg-[#0078d4] text-white" : ""}`}>Rocznie -20%</button></div></div><div className="mx-auto grid max-w-7xl gap-4 md:grid-cols-2 xl:grid-cols-4">{plans.map((p, i) => <motion.div key={p.name} whileHover={{ y: -8, scale: 1.01 }} initial={{ opacity: 0, y: 26 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: (i % 4) * 0.06 }} className={`relative rounded-[30px] border p-6 backdrop-blur-2xl ${p.featured ? "border-[#0078d4] bg-[#0078d4]/14 shadow-[0_26px_90px_rgba(0,120,212,0.20)]" : palette.card}`}>{p.featured && <div className="absolute -top-3 left-6 rounded-full bg-[#0078d4] px-4 py-1.5 text-xs font-bold text-white">NAJLEPSZY WYBĂ“R</div>}<div className="flex items-start justify-between gap-3"><div><h3 className="text-2xl font-semibold tracking-tight">{p.name}</h3><p className={`mt-2 text-sm leading-6 ${palette.muted}`}>{p.desc}</p></div><span className="rounded-full bg-[#0078d4]/15 px-3 py-1 text-xs font-semibold text-[#50a7f2]">{p.tag}</span></div><div className="mt-6"><div className="text-4xl font-semibold tracking-tight">{price(p.monthly)}</div>{p.monthly !== null && <div className={`mt-1 text-xs ${palette.faint}`}>/ miesiÄ…c {annual && p.monthly > 0 ? "przy pĹ‚atnoĹ›ci rocznej" : ""}</div>}</div><div className={`mt-4 rounded-2xl border p-3 text-xs font-semibold ${palette.card2}`}>{p.limits}</div><div className="mt-6 grid gap-3">{p.features.map((x) => <div key={x} className="flex items-start gap-2 text-sm leading-5"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-400" /> {x}</div>)}</div><button onClick={goAuth} className={`mt-7 inline-flex w-full items-center justify-center gap-2 rounded-full px-5 py-3 text-sm font-semibold transition ${p.featured ? "bg-[#0078d4] text-white hover:bg-[#106ebe]" : "border border-[#0078d4]/30 text-[#50a7f2] hover:bg-[#0078d4]/10"}`}>{p.cta} <ArrowRight className="h-4 w-4" /></button></motion.div>)}</div></section>;
}
