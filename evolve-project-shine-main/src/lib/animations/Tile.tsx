...
type TileSize = "1x1" | "2x1" | "2x2" | "3x1" | "3x2";
...
const tileEntry = {
  ...
  hover: { y: -4, scale: 1.02, transition: { type: "spring", stiffness: 400, damping: 20 } },
};
...
export function Tile({ size = "1x1", variant = "glass", className = "", children, index = 0 }: TileProps) {
  return (
    ...
    >
      <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none bg-[radial-gradient(ellipse_at_50%_0%,_oklch(0.85_0.18_160_/_0.08),_transparent_60%)]" />
      {children}
    </motion.div>
  );
}
