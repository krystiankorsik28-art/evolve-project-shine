      ...
      <shaderMaterial
        ...
        fragmentShader={`
          ...
          void main() {
            ...
            vec3 col = mix(vec3(0.0, 1.0, 0.53), vec3(1.0), bar);
...
function LogoShape({ show }: { show: boolean }) {
  ...
  useFrame((state, delta) => {
    ...
    const rotSpeed = 0.3 + Math.max(0, 1 - s) * 0.5;
    if (icoRef.current) {
      icoRef.current.rotation.x += delta * rotSpeed * 0.5;
      icoRef.current.rotation.y += delta * rotSpeed;
      icoRef.current.scale.setScalar(s);
    }
    if (torusRef.current) {
      torusRef.current.rotation.x += delta * rotSpeed * 0.3;
      torusRef.current.rotation.z += delta * rotSpeed * 0.4;
  ...
  return (
    <group visible={show}>
      <mesh ref={icoRef}>
        ...
        <meshBasicMaterial color="#00ff88" wireframe transparent opacity={opacity * 0.3} />
      </mesh>
      <mesh ref={torusRef}>
        <torusKnotGeometry args={[3.8, 1, 64, 16]} />
        <meshBasicMaterial color="#4488ff" wireframe transparent opacity={opacity * 0.2} />
      </mesh>
      <mesh ref={ringRef} rotation-x={Math.PI / 2}>
        <ringGeometry args={[3.2, 4.5, 64]} />
        <meshBasicMaterial color="#00ff88" transparent opacity={opacity * 0.08} side={THREE.DoubleSide} />
      </mesh>
    </group>
  );
}
...
function SceneContent({ onPhaseChange }: { onPhaseChange: (p: number) => void }) {
  ...
  useFrame((_, delta) => {
    ...
    if (t < 0.3) newPhase = 0;
    else if (t < 0.8) newPhase = 1;
    else if (t < 2.2) newPhase = 2;
    else if (t < 4.0) newPhase = 3;
    else newPhase = 4;
  ...
  const c = [
    ...
    () => ({ progress: (timeRef.current - 0.8) / 1.4, glitch: Math.max(0, 1 - (timeRef.current - 0.8) * 2), showLogo: false }),
    ...
    >
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 pointer-events-none">
        <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-white/[0.04] border border-white/[0.06] backdrop-blur-sm">
          ...
          <span className="text-[10px] text-fg-muted tracking-widest uppercase">
            {phase < 2 ? "Initializing" : phase < 4 ? "Rendering" : "Complete"}
          </span>
        </div>
      </div>
      {skipable && (
        <div className="absolute top-8 right-8 z-10 pointer-events-none">
          <span className="text-[10px] text-fg-subtle tracking-wider">Click anywhere to skip</span>
        </div>
      )}
      <Canvas
