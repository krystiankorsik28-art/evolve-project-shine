...
import { Sparkles, ArrowRight, Mail, Lock, Shield } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#0a0a12] flex">
      <div className="hidden lg:flex w-1/2 relative overflow-hidden bg-gradient-to-br from-[#0c0c1a] via-[#0a0a12] to-[#0c0c1a] p-12 flex-col justify-between">
        ...
        <div className="relative">
          <h2 className="text-3xl font-bold tracking-tight mb-4">
            The AI Platform<br />for Modern Education
          </h2>
          <p className="text-sm text-white/40 max-w-sm leading-relaxed mb-8">
            Generate exams, tutor students with AI, and track progress in real-time. One platform for the entire school ecosystem.
          </p>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8">
        ...
        >
          ...
          <p className="text-sm text-white/40 mb-6">Choose your role to continue</p>
            ...
            {ROLES.map((r) => (
              <button
                ...
                className={`px-3 py-1.5 text-xs rounded-md transition-all ${
                  role === r.id
                    ? "bg-white text-[#0a0a12] font-medium"
          ...
          <div className="flex gap-1 p-0.5 rounded-lg bg-white/[0.04] mb-6">
            <button
              ...
              className={`flex-1 px-3 py-1.5 text-xs rounded-md transition-all ${
                tab === "login" ? "bg-white text-[#0a0a12]" : "text-white/40"
              }`}
            >
              Sign in
            </button>
            <button
              onClick={() => setTab("pin")}
              className={`flex-1 px-3 py-1.5 text-xs rounded-md transition-all ${
                tab === "pin" ? "bg-white text-[#0a0a12]" : "text-white/40"
              }`}
            >
              PIN
            </button>
          ...
          {tab === "login" ? (
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2">
                  <input type="checkbox" className="w-3 h-3 rounded border-white/[0.08] bg-white/[0.04]" />
                  <span className="text-[10px] text-white/30">Remember me</span>
                </label>
                <a href="/auth/forgot-password" className="text-[10px] text-cyan-400/60 hover:text-cyan-400">Forgot password?</a>
              </div>
            ...
            <div className="space-y-4 text-center">
              <div className="text-sm text-white/40">Enter the PIN provided by your teacher</div>
              <div className="flex justify-center gap-2">
                {[0, 1, 2, 3, 4, 5].map((i) => (
                  <input key={i} maxLength={1} className="w-10 h-12 text-center text-base font-bold rounded-lg bg-white/[0.04] border border-white/[0.08] text-white outline-none focus:border-cyan-500/40 transition-colors" />
                ))}
              </div>
              <button className="w-full px-4 py-2.5 text-xs font-medium bg-gradient-to-r from-cyan-400 to-blue-500 text-white rounded-lg hover:shadow-lg hover:shadow-cyan-500/20 transition-all">
                Join exam
              </button>
              <p className="text-[10px] text-white/20">
                Don&apos;t have an account?{' '}
                <a href="/auth/register" className="text-cyan-400/60 hover:text-cyan-400">Sign up after exam</a>
              </p>
            </div>
          )}
          ...
          <p className="mt-6 text-center text-[10px] text-white/20">
            Don&apos;t have an account?{' '}
            <a href="/auth/register" className="text-cyan-400/60 hover:text-cyan-400">Create one</a>
          </p>
        </motion.div>
      </div>
    </div>
